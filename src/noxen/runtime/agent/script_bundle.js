...
`;
  ...
  var SENDING_TARGET_TYPE = {
    ...
    "sendOrderedBroadcast": "receiver",
    "getBroadcast": "receiver"
  };
  ...
  rpc.exports = {
    ...
    stageMod: function(type, key, val, extraType, decisionId) {
      if (matchesActiveDecision(decisionId)) {
        modQueue.push({ type, key, val, extraType });
        ...
        try {
          let enumComponents = function(arr, type) {
            ...
            for (var i = 0; i < arr.length; i++) {
              var ci = arr[i];
              var name = tryGet(function() {
                return String(ci.name.value);
              });
              var c = {
                name,
                type,
                ...
                enabledRuntime: tryGet(function() {
                  return pm.getComponentEnabledSetting(ComponentName.$new(pkg, name));
                }),
                processName: tryGet(function() {
                  return s(ci.processName.value);
                }),
                directBootAware: tryGet(function() {
                  return ci.directBootAware.value;
                })
              };
              if (type === "activity") {
                c.launchMode = tryGet(function() {
                  return ci.launchMode.value;
                });
                c.taskAffinity = tryGet(function() {
                  return s(ci.taskAffinity.value);
                });
                c.targetActivity = tryGet(function() {
                  return s(ci.targetActivity.value);
                });
              } else if (type === "service") {
                c.foregroundServiceType = tryGet(function() {
                  return ci.foregroundServiceType.value;
                });
          ...
          out.identity = {
            package: pkg,
            label: tryGet(function() {
              return String(pm.getApplicationLabel(ai));
            }),
            versionName: tryGet(function() {
              return s(pi.versionName.value);
            }),
            versionCode: tryGet(function() {
              return sdkInt >= 28 ? String(pi.getLongVersionCode()) : String(pi.versionCode.value);
            }),
            uid: tryGet(function() {
              return ai.uid.value;
            }),
            pid: Process.id,
            processName: tryGet(function() {
              var n = ActivityThread.currentProcessName();
              return n ? String(n) : null;
            }),
            sharedUserId: tryGet(function() {
              return s(pi.sharedUserId.value);
            }),
            ...
            firstInstallTime: tryGet(function() {
              return pi.firstInstallTime.value;
            }),
            lastUpdateTime: tryGet(function() {
              return pi.lastUpdateTime.value;
            })
          };
          var f = ai.flags.value;
          out.build = {
            deviceSdk: sdkInt,
            targetSdk: tryGet(function() {
              return ai.targetSdkVersion.value;
            }),
            minSdk: tryGet(function() {
              return ai.minSdkVersion.value;
            }),
            compileSdk: tryGet(function() {
              return ai.compileSdkVersion.value;
            }),
            debuggable: (f & 2) !== 0,
            allowBackup: (f & 32768) !== 0,
            testOnly: (f & 256) !== 0,
            extractNativeLibs: (f & 268435456) !== 0,
            cleartextPermitted: tryGet(function() {
              return Java.use("android.security.NetworkSecurityPolicy").getInstance().isCleartextTrafficPermitted();
            }),
            nscPresent: tryGet(function() {
              return ai.networkSecurityConfigRes.value !== 0;
            })
          };
